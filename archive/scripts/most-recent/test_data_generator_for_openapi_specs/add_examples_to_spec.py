...
def add_example_to_schema(schema: Dict[str, Any], prop_name: str = None) -> Dict[str, Any]:
    ...
    if 'oneOf' in schema:
        # For oneOf, we'll add examples to each option and pick the first one as the example
        for i, option in enumerate(schema['oneOf']):
            # Only process non-$ref schemas
            if '$ref' not in option:
                schema['oneOf'][i] = add_example_to_schema(option, prop_name)
        
        # Try to find the first option with an example
        for option in schema['oneOf']:
            if 'example' in option:
                schema['example'] = option['example']
                break
            # If it's an object with properties, generate an example
            elif option.get('type') == 'object' and 'properties' in option:
                temp_schema = add_example_to_schema(option.copy(), prop_name)
                if 'example' in temp_schema:
                    schema['example'] = temp_schema['example']
                    break
        return schema
    
    if 'anyOf' in schema:
        # For anyOf, similar to oneOf
        for i, option in enumerate(schema['anyOf']):
            # Only process non-$ref schemas
            if '$ref' not in option:
                schema['anyOf'][i] = add_example_to_schema(option, prop_name)
    ...
    if 'allOf' in schema:
        ...
        for i, option in enumerate(schema['allOf']):
            # Only process non-$ref schemas
            if '$ref' not in option:
                schema['allOf'][i] = add_example_to_schema(option, prop_name)
                if 'example' in schema['allOf'][i]:
                    if isinstance(schema['allOf'][i]['example'], dict):
                        merged_example.update(schema['allOf'][i]['example'])
                    else:
                        # If we get a non-dict example, use it if we don't have one yet
                        if not merged_example:
                            schema['example'] = schema['allOf'][i]['example']
                            return schema
        if merged_example:
...
def process_openapi_spec(spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if 'components' in spec and 'schemas' in spec['components']:
        for k, schema in spec['components']['schemas'].items():
            spec['components']['schemas'][k] = add_example_to_schema(schema, k)
