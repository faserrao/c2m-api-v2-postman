  ...
  class SplitPdfParamsRequestItemsInner
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::SplitPdfParamsRequestItemsInner` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::SplitPdfParamsRequestItemsInner`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # @param [Object] page_range Value to be assigned
    def page_range=(page_range)
      if page_range.nil?
        fail ArgumentError, 'page_range cannot be nil'
      end
    ...
    def hash
      [page_range, recipient_address_sources].hash
    end
