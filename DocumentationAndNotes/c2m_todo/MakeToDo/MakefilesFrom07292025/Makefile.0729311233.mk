...
# === DEBUG FILES ===
MOCKS_DEBUG_FILE        := $(POSTMAN_DIR)/mocks-debug.json
COLLECTIONS_DEBUG_FILE  := $(POSTMAN_DIR)/collections-debug.json
APIS_DEBUG_FILE         := $(POSTMAN_DIR)/apis-debug.json
ENVIRONMENTS_DEBUG_FILE := $(POSTMAN_DIR)/environments-debug.json
SPECS_DEBUG_FILE        := $(POSTMAN_DIR)/specs-debug.json
...
# === COLLECTION FILES (Consistent naming) ===
...
COLLECTION_TMP_FILE       := $(POSTMAN_GEN_DIR)/c2mapiv2-collection.tmp.json
...
postman-apis: ## List all Postman APIs
	...
	curl --silent --location $(POSTMAN_HEADER_API_KEY) \
	"$(POSTMAN_API_BASE_URL)/apis" | jq .
...
postman-api-import:
	...
	@API_RESPONSE=$$(curl --location --request POST "$(POSTMAN_APIS_URL)&workspaceId=$(POSTMAN_WS)" \
...
postman-api-full-publish:
	...
	@SPECS=$$(curl --silent $(POSTMAN_HEADER_API_KEY) \
		"$(POSTMAN_SPECS_URL)" \
		| jq -r '.data[].id'); \
	if [ -n "$$SPECS" ]; then \
		...
		for ID in $$SPECS; do \
			...
			curl --silent --location \
				--request DELETE \
				"$(POSTMAN_API_BASE_URL)/specs/$$ID" \
				$(POSTMAN_CURL_HEADERS) | jq .; \
		done; \
	else \
	...
	jq -n \
		...
		> $(POSTMAN_DIR)/full-publish-payload.json; \
	curl --silent \
		--location \
		--request POST \
		"$(POSTMAN_SPECS_URL)" \
		$(POSTMAN_CURL_HEADERS) \
		--data @$(POSTMAN_DIR)/full-publish-payload.json \
		| tee $(POSTMAN_DIR)/api-full-publish-response.json | jq .; \
	SPEC_ID=$$(jq -r '.id // empty' $(POSTMAN_DIR)/api-full-publish-response.json); \
	if [ -z "$$SPEC_ID" ]; then \
		echo "❌ Failed to create a fresh spec. See $(POSTMAN_DIR)/api-full-publish-response.json."; \
		exit 1; \
	else \
...
postman-api-update:
	...
	jq -n \
		...
		> $(POSTMAN_DIR)/update-payload.json; \
	curl --silent \
		--location \
		--request POST \
		"$(POSTMAN_API_BASE_URL)/specs/$$SPEC_ID/files" \
		$(POSTMAN_CURL_HEADERS) \
		--data @$(POSTMAN_DIR)/update-payload.json \
		| tee $(POSTMAN_DIR)/api-update-response.json | jq .
...
postman-collection-upload:
	...
	@COLL_UID=$$(jq -c '{collection: .}' $(COLLECTION_RAW) | \
		curl --silent --location --request POST "$(POSTMAN_API_BASE_URL)/collections?workspace=$(POSTMAN_WS)" \
			$(POSTMAN_CURL_HEADERS) \
...
postman-collection-link:
	...
	jq -n --arg coll "$$COLL_UID" '{operationType: "COPY_COLLECTION", data: {collectionId: $$coll}}' > $(POSTMAN_LINK_PAYLOAD); \
...
postman-collection-fix:
	...
	@python3 $(SCRIPTS_DIR)/fix_collection_urls.py $(COLLECTION_FIXED) $(COLLECTION_FIXED)
...
postman-collection-fix-v2:
	...
	@python3 $(SCRIPTS_DIR)/fix_collection_urls_v2.py $(COLLECTION_FIXED) $(COLLECTION_FIXED)
...
postman-collection-url-hardfix:
	@echo "🔧 Applying combined path + URL hard fix..."
		jq -f $(SCRIPTS_DIR)/url_hardfix_with_paths.jq $(COLLECTION_FIXED) > $(COLLECTION_FIXED).tmp \
...
postman-collection-repair-urls:
	@echo "🔧 Repairing URLs based on folder hierarchy..."
		python3 $(SCRIPTS_DIR)/repair_urls.py $(COLLECTION_FIXED)
...
postman-collection-upload-test:
	...
	@RESPONSE=$$(jq -c '{collection: .}' $(COLLECTION_FIXED) | \
		curl --silent --location --request POST "$(POSTMAN_API_BASE_URL)/collections?workspace=$(POSTMAN_WS)" \
			$(POSTMAN_CURL_HEADERS) \
			--data-binary @-); \
		echo "$$RESPONSE" | jq . > $(POSTMAN_DIR)/upload-test-debug.json || echo "$$RESPONSE" > $(POSTMAN_DIR)/upload-test-debug.json; \
		COLL_UID=$$(echo "$$RESPONSE" | jq -r '.collection.uid // empty'); \
		if [ -z "$$COLL_UID" ] || [ "$$COLL_UID" = "null" ]; then \
			echo "❌ Failed to upload test collection. Check $(POSTMAN_DIR)/upload-test-debug.json for details."; \
			exit 1; \
		else \
			echo "✅ TEST Collection uploaded with UID: $$COLL_UID"; \
			echo $$COLL_UID > $(POSTMAN_DIR)/postman_test_collection_uid.txt; \
			echo "📄 UID saved to $(POSTMAN_DIR)/postman_test_collection_uid.txt"; \
		fi
...
sync-mock:
	...
	@if [ -z "$(MOCK_UID)" ]; then \
		...
		curl --silent --location --request POST "$(POSTMAN_API_BASE_URL)/mocks" \
	...
	curl --location --request PUT "$(POSTMAN_API_BASE_URL)/mocks/$(shell cat $(MOCK_UID_FILE))" \
		$(POSTMAN_HEADER_API_KEY) \
		$(POSTMAN_HEADER_CONTENT_TYPE) \
		--data-raw "{\"mock\": { \
			\"name\": \"Auto Mock\", \
...
postman-env-upload:
	...
	@RESPONSE=$$(curl --silent --location --request POST "$(POSTMAN_API_BASE_URL)/environments?workspace=$(POSTMAN_WS)" \
		$(POSTMAN_CURL_HEADERS) \
		--data-binary '@$(ENV_FILE)'); \
	echo "$$RESPONSE" | jq . > $(POSTMAN_DIR)/env-upload-debug.json || echo "$$RESPONSE" > $(POSTMAN_DIR)/env-upload-debug.json; \
	ENV_UID=$$(echo "$$RESPONSE" | jq -r '.environment.uid // empty'); \
	if [ -z "$$ENV_UID" ]; then \
		echo "❌ Failed to upload environment. Check $(POSTMAN_DIR)/env-upload-debug.json for details."; \
		exit 1; \
	else \
		echo "✅ Environment uploaded with UID: $$ENV_UID"; \
		echo $$ENV_UID > $(ENV_UID_FILE); \
	fi

update-mock-env:
	@echo "🔄 Updating Postman mock server environment..."
	curl --location --request PUT "$(POSTMAN_API_BASE_URL)/mocks/$(MOCK_ID)" \
		$(POSTMAN_CURL_HEADERS) \
		--data-raw "{\"mock\": { \
...
verify-mock:
	...
	@curl --silent --location --request GET "$(POSTMAN_API_BASE_URL)/mocks/$(MOCK_UID)" \
		$(POSTMAN_HEADER_API_KEY) \
		| jq '{ \
...
postman-mock-create:
	...
	@if [ ! -f $(POSTMAN_DIR)/postman_test_collection_uid.txt ]; then \
		echo "❌ Missing test collection UID file: $(POSTMAN_DIR)/postman_test_collection_uid.txt. Run postman-collection-upload-test first."; \
		exit 1; \
	fi; \
	COLL_UID=$$(cat $(POSTMAN_DIR)/postman_test_collection_uid.txt); \
	...
	jq -n --arg coll "$$COLL_UID" --arg name "$$MOCK_NAME" \
		...
		> $(POSTMAN_DIR)/mock-payload.json; \
	echo "📤 Creating mock server via Postman API..."; \
	curl --silent --location --request POST "$(POSTMAN_API_BASE_URL)/mocks" \
		$(POSTMAN_CURL_HEADERS) \
		--data-binary @$(POSTMAN_DIR)/mock-payload.json \
		-o $(POSTMAN_DIR)/mock-debug.json; \
	if ! jq -e '.mock.mockUrl' $(POSTMAN_DIR)/mock-debug.json >/dev/null; then \
		echo "❌ Failed to create mock server. See $(POSTMAN_DIR)/mock-debug.json"; \
		exit 1; \
	fi; \
	POSTMAN_MOCK_URL=$$(jq -r '.mock.mockUrl' $(POSTMAN_DIR)/mock-debug.json); \
	MOCK_UID=$$(jq -r '.mock.uid' $(POSTMAN_DIR)/mock-debug.json | sed 's/^[^-]*-//'); \
	echo "✅ Mock server created at: $$POSTMAN_MOCK_URL"; \
	echo "📄 Saving mock URL and UID..."; \
	echo "$$POSTMAN_MOCK_URL" > $(MOCK_URL_FILE_POSTMAN); \
	echo "$$MOCK_UID" > $(MOCK_UID_FILE); \
	echo "📄 Mock server URL saved to $(MOCK_URL_FILE_POSTMAN)"; \
	echo "📄 Mock UID saved to $(MOCK_UID_FILE)"; \
	echo "🔍 Validating mock configuration..."; \
	curl --silent --location --request GET "$(POSTMAN_API_BASE_URL)/mocks/$$MOCK_UID" \
		$(POSTMAN_HEADER_API_KEY) \
		-o $(POSTMAN_DIR)/mock-validate.json; \
	if jq -e '.error' $(POSTMAN_DIR)/mock-validate.json >/dev/null; then \
		echo "❌ Postman mock validation failed. See $(POSTMAN_DIR)/mock-validate.json"; \
		exit 1; \
	else \
...
postman-link-env-to-collection:
	...
	@if [ ! -f $(ENV_UID_FILE) ]; then \
		echo "❌ Missing environment UID file: $(ENV_UID_FILE). Run postman-env-upload first."; \
		exit 1; \
	fi
	@if [ ! -f $(POSTMAN_DIR)/postman_test_collection_uid.txt ]; then \
		echo "❌ Missing test collection UID file: $(POSTMAN_DIR)/postman_test_collection_uid.txt. Run postman-collection-upload-test first."; \
	...
	@if [ ! -f $(MOCK_UID_FILE) ]; then \
		echo "❌ Missing mock UID file: $(MOCK_UID_FILE). Run postman-mock-create first."; \
		exit 1; \
	fi
	ENV_UID=$$(cat $(ENV_UID_FILE)); \
	COLL_UID=$$(cat $(POSTMAN_DIR)/postman_test_collection_uid.txt); \
	MOCK_UID=$$(cat $(MOCK_UID_FILE)); \
	echo "📦 Linking Environment $$ENV_UID with Collection $$COLL_UID (Mock $$MOCK_UID)..."; \
	curl --silent --location --request PUT "$(POSTMAN_API_BASE_URL)/mocks/$$MOCK_UID" \
		$(POSTMAN_CURL_HEADERS) \
		...
		-o $(POSTMAN_DIR)/mock-link-debug.json; \
	if jq -e '.mock' $(POSTMAN_DIR)/mock-link-debug.json >/dev/null; then \
		echo "✅ Environment linked to mock server successfully."; \
	else \
		echo "❌ Failed to link environment. See $(POSTMAN_DIR)/mock-link-debug.json"; \
		exit 1; \
	fi
...
postman-api-list-specs:
	...
	@curl --silent $(POSTMAN_HEADER_API_KEY) \
		"$(POSTMAN_SPECS_URL)" \
...
postman-cleanup:
	...
	@if [ -f $(MOCK_UID_FILE) ]; then \
		...
		curl --silent --location --request DELETE "$(POSTMAN_API_BASE_URL)/mocks/$$MOCK_UID" \
			$(POSTMAN_CURL_HEADERS) \
			| tee $(POSTMAN_DIR)/mock-delete-debug.json; \
		echo "✅ Mock server deleted."; \
		rm -f $(MOCK_UID_FILE) $(MOCK_URL_FILE_POSTMAN); \
	else \
		echo "⚠️  No mock UID found at $(MOCK_UID_FILE)"; \
	fi
	...
	@if [ -f $(ENV_UID_FILE) ]; then \
		...
		curl --silent --location --request DELETE "$(POSTMAN_API_BASE_URL)/environments/$$ENV_UID" \
			$(POSTMAN_CURL_HEADERS) \
			| tee $(POSTMAN_DIR)/env-delete-debug.json; \
		echo "✅ Environment deleted."; \
		rm -f $(ENV_UID_FILE) $(ENV_FILE); \
	else \
		echo "⚠️  No environment UID found at $(ENV_UID_FILE)"; \
	fi
	...
	@if [ -f $(POSTMAN_DIR)/postman_test_collection_uid.txt ]; then \
		...
		curl --silent --location --request DELETE "$(POSTMAN_API_BASE_URL)/collections/$$COLL_UID" \
			$(POSTMAN_CURL_HEADERS) \
			| tee $(POSTMAN_DIR)/collection-delete-debug.json; \
		echo "✅ Test collection deleted."; \
		rm -f $(POSTMAN_DIR)/postman_test_collection_uid.txt; \
	else \
		echo "⚠️  No test collection UID found at $(POSTMAN_DIR)/postman_test_collection_uid.txt"; \
	fi
	...
	@if [ -f $(POSTMAN_COLLECTION_UID_FILE) ]; then \
		...
		curl --silent --location --request DELETE "$(POSTMAN_API_BASE_URL)/collections/$$COLL_UID" \
			$(POSTMAN_CURL_HEADERS) \
			| tee $(POSTMAN_DIR)/collection-main-delete-debug.json; \
		echo "✅ Main collection deleted."; \
		rm -f $(POSTMAN_COLLECTION_UID_FILE); \
	else \
		echo "⚠️  No main collection UID found at $(POSTMAN_COLLECTION_UID_FILE)"; \
	fi
...
postman-cleanup-all:
	...
	@MOCKS=$$(curl --silent --location "$(POSTMAN_MOCKS_URL)" \
		$(POSTMAN_HEADER_API_KEY) | tee $(MOCKS_DEBUG_FILE) | jq -r '.mocks // [] | .[].id'); \
	...
	@COLLECTIONS=$$(curl --silent --location "$(POSTMAN_COLLECTIONS_URL)" \
		$(POSTMAN_HEADER_API_KEY) | tee $(COLLECTIONS_DEBUG_FILE) | jq -r '.collections // [] | .[].uid'); \
	...
	# --- Delete APIs ---
	@echo "🔍 Fetching APIs..."
	@APIS=$$(curl --silent --location "$(POSTMAN_APIS_URL)" \
		$(POSTMAN_HEADER_API_KEY) | tee $(APIS_DEBUG_FILE) | jq -r '.apis // [] | .[].id'); \
	...
	@ENVS=$$(curl --silent --location "$(POSTMAN_ENVIRONMENTS_URL)" \
		$(POSTMAN_HEADER_API_KEY) | tee $(ENVIRONMENTS_DEBUG_FILE) | jq -r '.environments // [] | .[].uid'); \
	echo "DEBUG: ENVIRONMENTS=$$ENVS"; \
	if [ -z "$$ENVS" ]; then \
	...
	# --- Delete Specs ---
	@echo "🔍 Fetching specs..."
	@SPECS=$$(curl --silent --location "$(POSTMAN_SPECS_URL)" \
		$(POSTMAN_HEADER_API_KEY) | tee $(SPECS_DEBUG_FILE) | jq -r '.data // [] | .[].id'); \
...
postman-api-clean-trash:
	...
	@TRASH=$$(curl --silent $(POSTMAN_HEADER_API_KEY) \
		"$(POSTMAN_SPECS_URL)&status=trashed" \
		| tee $(POSTMAN_DIR)/trash-debug.json | jq -r '.data // [] | .[].id'); \
	echo "DEBUG: TRASH=$$TRASH"; \
	if [ -z "$$TRASH" ]; then \
		...
		for ID in $$TRASH; do \
			...
			curl --silent --location \
				--request DELETE \
				"$(POSTMAN_API_BASE_URL)/specs/$$ID?permanent=true" \
				$(POSTMAN_CURL_HEADERS) | jq .; \
		done; \
...
# === NEW DEBUG TARGET ===
.PHONY: postman-debug-all
postman-debug-all:
	@echo "🔍 === POSTMAN DEBUG ALL ==="
	@echo "Workspace ID: $(POSTMAN_WS)"
	@echo "API Key: $(POSTMAN_API_KEY)"
	@echo ""
	
	@echo "📋 === MOCKS ==="
	@curl --silent $(POSTMAN_HEADER_API_KEY) "$(POSTMAN_MOCKS_URL)" | \
		tee $(MOCKS_DEBUG_FILE) | \
		jq -r '.mocks // [] | .[] | "Name: \(.name)\nID: \(.id)\nURL: \(.mockUrl)\n---"'
	
	@echo "📋 === COLLECTIONS ==="
	@curl --silent $(POSTMAN_HEADER_API_KEY) "$(POSTMAN_COLLECTIONS_URL)" | \
		tee $(COLLECTIONS_DEBUG_FILE) | \
		jq -r '.collections // [] | .[] | "Name: \(.name)\nUID: \(.uid)\nFork: \(.fork // "N/A")\n---"'
	
	@echo "📋 === APIS ==="
	@curl --silent $(POSTMAN_HEADER_API_KEY) "$(POSTMAN_APIS_URL)" | \
		tee $(APIS_DEBUG_FILE) | \
		jq -r '.apis // [] | .[] | "Name: \(.name)\nID: \(.id)\nCreated: \(.createdAt)\n---"'
	
	@echo "📋 === ENVIRONMENTS ==="
	@curl --silent $(POSTMAN_HEADER_API_KEY) "$(POSTMAN_ENVIRONMENTS_URL)" | \
		tee $(ENVIRONMENTS_DEBUG_FILE) | \
		jq -r '.environments // [] | .[] | "Name: \(.name)\nUID: \(.uid)\n---"'
	
	@echo "📋 === SPECS ==="
	@curl --silent $(POSTMAN_HEADER_API_KEY) "$(POSTMAN_SPECS_URL)" | \
		tee $(SPECS_DEBUG_FILE) | \
		jq -r '.data // [] | .[] | "Name: \(.name)\nID: \(.id)\nType: \(.type)\nUpdated: \(.updatedAt)\n---"'
	
	@echo "✅ Debug files saved to $(POSTMAN_DIR)/*-debug.json"
