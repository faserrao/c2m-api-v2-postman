...
postman-api-import:
	@echo "📥 Importing OpenAPI spec $(OPENAPI_SPEC) into Postman workspace $(POSTMAN_WS) via /specs..."

	# Build payload JSON (same method as full-publish)
	...
	# Create or overwrite the spec
	@API_RESPONSE=$$(curl --silent --fail --location \
		--request POST "$(POSTMAN_SPECS_URL)?workspaceId=$(POSTMAN_WS)" \
		$(POSTMAN_CURL_HEADERS) \
		--data @$(POSTMAN_FULL_PAYLOAD) || echo ""); \
		if [ -z "$$API_RESPONSE" ]; then \
			echo "❌ API request failed."; \
			exit 1; \
		fi; \
		echo "$$API_RESPONSE" | jq . > $(POSTMAN_FULL_RESPONSE) || echo "$$API_RESPONSE" > $(POSTMAN_FULL_RESPONSE); \
		SPEC_ID=$$(jq -r '.id // empty' $(POSTMAN_FULL_RESPONSE)); \
		if [ -z "$$SPEC_ID" ]; then \
			echo "❌ Failed to import spec. Check $(POSTMAN_FULL_RESPONSE) for details."; \
			exit 1; \
		fi; \
		echo "✅ Spec imported with ID: $$SPEC_ID"; \
		echo "$$SPEC_ID" > $(POSTMAN_SPEC_ID_FILE); \
		echo "📄 Spec ID saved to $(POSTMAN_SPEC_ID_FILE)";
...
postman-collection-add-tests:
	@echo "🧪 Adding default Postman tests to $(COLLECTION_MERGED) with allowed codes: $(POSTMAN_ALLOWED_CODES)..."
...
prism-mock-test:
	...
	@echo "📦 Using collection: $(COLLECTION_FIXED:.json=.with.tests.json)"
...
prism-test-list:
	@echo "📋 Listing request bodies for endpoint: $(PRISM_TEST_ENDPOINT)..."
