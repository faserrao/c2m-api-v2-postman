...
def create_collection() -> Dict:
    ...
    collection = {
        "info": {
            ...
            "description": (
                "This collection demonstrates real-world use cases for the C2M API v2. "
                "Each folder represents a specific business scenario with pre-populated test data.\n\n"
    ...
    for use_case_key, use_case in USE_CASES.items():
        folder = {
            ...
            "item": [
                ...
                create_get_job_details_request(),
                create_get_job_status_request()
            ]
        }
...
def main():
    ...
    print(f"📊 Created {len(USE_CASES)} use cases with {len(USE_CASES) * 3} total requests")
