#!/usr/bin/env python3
"""
Extract Mini EBNF Data Dictionaries for Each Endpoint
=====================================================

This utility extracts all EBNF productions used by each endpoint and 
outputs them as mini data dictionaries in EBNF format.

Usage:
    python extract_endpoint_ebnf.py data_dictionary/c2mapiv2-dd.ebnf [-o output.txt]

Output:
    A mini EBNF data dictionary for each endpoint showing all productions
    that endpoint depends on.
"""

import argparse
import sys
import re
from pathlib import Path
from typing import Dict, List, Set, Optional, Tuple
from collections import OrderedDict

# Import the existing EBNF translator to leverage its parsing
sys.path.append(str(Path(__file__).parent))
from ebnf_to_openapi_dynamic_v2 import (
    EBNFToOpenAPITranslator, 
    EBNFProduction
)


class EndpointEBNFExtractor:
    """Extracts mini EBNF data dictionaries for each endpoint"""
    
    def __init__(self):
        self.translator = EBNFToOpenAPITranslator()
        self.ebnf_lines: Dict[str, List[str]] = {}  # production name -> original EBNF lines
        self.comments: Dict[str, str] = {}  # production name -> associated comment
        
    def analyze_file(self, filepath: str) -> Dict[str, str]:
        """Analyze EBNF file and return mini dictionaries for each endpoint"""
        with open(filepath, 'r') as f:
            content = f.read()
        
        # Parse to get productions
        self.translator.parse_ebnf(content)
        
        # Extract original EBNF definitions and comments
        self._extract_original_definitions(content)
        
        # Map endpoints to their parameter productions
        endpoint_to_params = self._map_endpoints_to_params(content)
        
        # Build mini dictionaries for each endpoint
        endpoint_dictionaries = {}
        
        for endpoint_path, param_name in endpoint_to_params.items():
            if param_name in self.translator.productions:
                # Get all productions used by this endpoint
                used_productions = self._get_all_dependencies(param_name)
                
                # Build the mini dictionary
                mini_dict = self._build_mini_dictionary(
                    endpoint_path, 
                    param_name,
                    used_productions
                )
                endpoint_dictionaries[endpoint_path] = mini_dict
        
        return endpoint_dictionaries
    
    def _extract_original_definitions(self, content: str):
        """Extract the original EBNF definitions from the file"""
        lines = content.split('\n')
        
        i = 0
        while i < len(lines):
            line = lines[i]
            
            # Skip empty lines and pure comments
            if not line.strip() or (line.strip().startswith('(*') and line.strip().endswith('*)')):
                i += 1
                continue
            
            # Look for production definitions
            match = re.match(r'^(\w+)\s*=', line)
            if match:
                prod_name = match.group(1)
                definition_lines = []
                
                # Check for preceding comment
                if i > 0 and '(*' in lines[i-1]:
                    comment_match = re.search(r'\(\*\s*(.+?)\s*\*\)', lines[i-1])
                    if comment_match:
                        self.comments[prod_name] = comment_match.group(1)
                
                # Collect all lines of this production (until we find ';')
                j = i
                while j < len(lines):
                    definition_lines.append(lines[j])
                    if ';' in lines[j]:
                        break
                    j += 1
                
                self.ebnf_lines[prod_name] = definition_lines
                i = j + 1
            else:
                i += 1
    
    def _map_endpoints_to_params(self, content: str) -> Dict[str, str]:
        """Map endpoint paths to their parameter production names"""
        lines = content.split('\n')
        endpoint_to_params = {}
        
        endpoint_pattern = r'Endpoint:\s*(?:GET|POST|PUT|DELETE|PATCH)\s+(/[\w/\-{}]+)'
        
        for i, line in enumerate(lines):
            match = re.search(endpoint_pattern, line, re.IGNORECASE)
            if match:
                endpoint_path = match.group(1)
                
                # Look for the production immediately following
                for j in range(i + 1, min(i + 10, len(lines))):
                    if lines[j].strip() and not lines[j].strip().startswith('*)'):
                        prod_match = re.match(r'^(\w+)\s*=', lines[j])
                        if prod_match:
                            param_name = prod_match.group(1)
                            endpoint_to_params[endpoint_path] = param_name
                            break
        
        return endpoint_to_params
    
    def _get_all_dependencies(self, production_name: str, 
                            visited: Optional[Set[str]] = None) -> Set[str]:
        """Get all productions that a given production depends on"""
        if visited is None:
            visited = set()
        
        if production_name in visited:
            return set()
        
        visited.add(production_name)
        dependencies = {production_name}
        
        # Get the production
        if production_name not in self.translator.productions:
            return dependencies
        
        production = self.translator.productions[production_name]
        
        # Find all symbols referenced in the expression
        symbols = self._extract_symbols_from_expression(production.expression)
        
        # Recursively get dependencies
        for symbol in symbols:
            if symbol in self.translator.productions:
                sub_deps = self._get_all_dependencies(symbol, visited)
                dependencies.update(sub_deps)
        
        return dependencies
    
    def _extract_symbols_from_expression(self, expr) -> Set[str]:
        """Extract all symbol names from an expression"""
        symbols = set()
        
        if isinstance(expr, dict):
            expr_type = expr.get('type')
            
            if expr_type == 'symbol':
                name = expr.get('name')
                if name and isinstance(name, str):
                    symbols.add(name)
            
            # Recursively process nested structures
            for key, value in expr.items():
                if key != 'type':
                    if isinstance(value, list):
                        for item in value:
                            symbols.update(self._extract_symbols_from_expression(item))
                    elif isinstance(value, dict):
                        symbols.update(self._extract_symbols_from_expression(value))
        
        elif isinstance(expr, list):
            for item in expr:
                symbols.update(self._extract_symbols_from_expression(item))
        
        return symbols
    
    def _build_mini_dictionary(self, endpoint_path: str, 
                              main_production: str,
                              used_productions: Set[str]) -> str:
        """Build a mini EBNF dictionary for an endpoint"""
        lines = []
        
        # Header
        lines.append(f"(* ===== Mini Data Dictionary for {endpoint_path} ===== *)")
        lines.append(f"(* Main production: {main_production} *)")
        lines.append("")
        
        # Sort productions for consistent output
        # Put main production first, then sort others
        sorted_prods = [main_production] + sorted(used_productions - {main_production})
        
        # Add each production
        for prod_name in sorted_prods:
            if prod_name in self.ebnf_lines:
                # Add comment if exists
                if prod_name in self.comments:
                    lines.append(f"(* {self.comments[prod_name]} *)")
                
                # Add the production definition
                for line in self.ebnf_lines[prod_name]:
                    lines.append(line)
                
                lines.append("")  # Empty line between productions
        
        return '\n'.join(lines).strip()
    
    def format_all_endpoints(self, endpoint_dictionaries: Dict[str, str]) -> str:
        """Format all endpoint mini dictionaries"""
        lines = ["# Endpoint EBNF Data Dictionaries", ""]
        lines.append("This document contains mini EBNF data dictionaries for each API endpoint.")
        lines.append("")
        
        for endpoint_path in sorted(endpoint_dictionaries.keys()):
            lines.append(f"## {endpoint_path}")
            lines.append("")
            lines.append("```ebnf")
            lines.append(endpoint_dictionaries[endpoint_path])
            lines.append("```")
            lines.append("")
        
        # Add summary
        lines.append("## Summary")
        lines.append("")
        
        # Count unique productions per endpoint
        production_counts = {}
        for endpoint_path, mini_dict in endpoint_dictionaries.items():
            # Count productions in the mini dictionary
            count = len(re.findall(r'^\w+\s*=', mini_dict, re.MULTILINE))
            production_counts[endpoint_path] = count
        
        lines.append("### Production Count by Endpoint")
        lines.append("")
        for endpoint, count in sorted(production_counts.items()):
            lines.append(f"- {endpoint}: {count} productions")
        
        return '\n'.join(lines)


def main():
    parser = argparse.ArgumentParser(
        description="Extract mini EBNF data dictionaries for each endpoint"
    )
    parser.add_argument("input", help="Input EBNF file")
    parser.add_argument("-o", "--output", help="Output file (default: stdout)")
    parser.add_argument("-f", "--format", choices=["ebnf", "markdown"], 
                       default="markdown", help="Output format")
    
    args = parser.parse_args()
    
    # Check input file
    if not Path(args.input).exists():
        print(f"Error: Input file '{args.input}' not found", file=sys.stderr)
        sys.exit(1)
    
    # Extract
    extractor = EndpointEBNFExtractor()
    try:
        endpoint_dictionaries = extractor.analyze_file(args.input)
    except Exception as e:
        print(f"Error analyzing file: {e}", file=sys.stderr)
        import traceback
        traceback.print_exc()
        sys.exit(1)
    
    # Format output
    if args.format == "markdown":
        output = extractor.format_all_endpoints(endpoint_dictionaries)
    else:  # ebnf
        # Just concatenate all mini dictionaries
        output = "\n\n".join(
            f"(* ========== {endpoint} ========== *)\n{mini_dict}"
            for endpoint, mini_dict in sorted(endpoint_dictionaries.items())
        )
    
    # Write output
    if args.output:
        with open(args.output, 'w') as f:
            f.write(output)
        print(f"Mini dictionaries written to: {args.output}")
    else:
        print(output)


if __name__ == "__main__":
    main()